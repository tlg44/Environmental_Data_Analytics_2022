...
ozone_combined_dataset$Date <- as.Date(ozone_combined_dataset$Date, format = "%m/%d/%Y")
...
ozone.line.plot <- ggplot(GaringerOzone, aes(y = Daily.Max.8.hour.Ozone.Concentration, x = Date), 
            size = 0.25) + 
  geom_line() + 
...
```{r}
#8
GaringerOzoneNA <- GaringerOzone %>% mutate(Daily.Max.8.hour.Ozone.Concentration.clean = zoo::na.approx(Daily.Max.8.hour.Ozone.Concentration))

#plotting to see how its interpolated
ggplot(GaringerOzoneNA) + 
...
#old notes from lesson
# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #use this if your NA is not at the begining or end of your data set - can't use NA.omit
...
GaringerOzone.groupings <- GaringerOzone %>% add_column(as.vector(year), as.vector(month))
