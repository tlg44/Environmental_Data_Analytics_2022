...
scatter.temp.depth <- ggplot(NTL.LTER.JUL, aes(x = temperature_C , y = depth)) + 
  geom_point() + geom_smooth(method = lm) + ylim(0, 35) + scale_y_reverse() 

#everytime I add the lm line, the y lim no longer works properly. 
#or once I reverse the 7 scale. 
...
```{r temperature.model}
#9 AIC
temp.depth.AIC <- lm(data = NTL.LTER.JUL, temperature_C ~ depth + year4 + daynum)
