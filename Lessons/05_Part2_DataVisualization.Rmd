...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #with x and y lines an no gridlines
print(O3plot4)
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #font size = base size
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #horizontal line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + 
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # hjust = 1 means left align (this shows where to put the text on the graph, around the set date)
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #whats the first and last date you want plotted
    date_breaks = "2 months", date_labels = "%b %y") + # the b is abreviated month name
  ylab(expression("O"[3]* " AQI Value")) + #change the y axis title the brakets around the 3 mean it is subscript
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #change the angle of the x axis text
print(O3plot7)  
...
NvsP2 <-
  ...
  #(the alpha adds some transparency)
  scale_shape_manual(values = c(15, 17)) + #this is triangles and square shapes with 15 and 17
...
#shows multiple plots in same window number of rows is number of plots 
```
