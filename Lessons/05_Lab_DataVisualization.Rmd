...
SurfaceTempPlot <-
  ...
  geom_dotplot(aes(color = temperature_C, fill = temperature_C), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) 
print(SurfaceTempPlot)


SurfaceTempPlot2 <- ggplot(PeterPaul.chem.nutrients, aes(x = daynum, y = temperature_C, col = year4)) +
  geom_point() + facet_wrap(vars(lakename)) +
  ylab("Day of the Year")
print(SurfaceTempPlot2)

SurfaceTempPlot3 <- ggplot(filter(PeterPaul.chem.nutrients, depth ==0)) +
  geom_point(aes(x = daynum, y = temperature_C, color=year4)) +
  facet_wrap('lakename') +
  ylab('Temerature(C)')
print(SurfaceTempPlot3)
...
Tempbydateplot <- ggplot(PeterPaul.chem.nutrients) + 
  geom_point(aes( x = sampledate, y = temperature_C, color = depth), size = .5) + 
  scale_color_viridis(option = 'magma', direction = 1) +
  scale_x_date(date_breaks = '5 years', date_labels = '%Y')
print(Tempbydateplot)
