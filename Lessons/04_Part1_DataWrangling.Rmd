...
`!=` #check if varialbes are different
`<` less than
`<=`
`>`
`>=` greater or equal
`&` all should be true
`|` one or other should be true

```{r}
class(NTL.phys.data$lakeid)
class(NTL.phys.data$depth)

# matrix filtering for only rows with zero on depth column
...
# Choose multiple conditions to filter (the | is an 'or' command) (%in% is way to filter for specific names) (all do the same thing)
...
# Choose a range of conditions of a numeric or integer variable (use & so both conditions must be true) (all below do the same thing)
...
> Answer: b/c row numbers can change when data is filtered/re-arranged. 
...
# use the : to say this column through this end column
...
# after the comma is the name of the new column you are adding into the dataframe
...
# find out the start and end dates of the dataset (both of these do the same thing)
