...
EPAair <- read.csv("./Data/Processed/EPAair_03_PM25_NC2022_Processed.csv", stringsAsFactors = TRUE)

EPAair <- EPAair_O3_PM25_NC2122_Processed
...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) This one added a layer to plot with a title to the chart with the color blue as title
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot outliers are making the visualization difficult
lightvsDO <- 
...
# Depth in the fields of limnology and oceanography is on a reverse scale (you want zero on top and lower depths at bottom)
tempvsdepth <- 
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #this adds in a trend line the lm means linear function
  geom_abline(aes(slope = 16, intercept = 0)) #this adds in line based on the info you knew
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill" and used color instead? The fill keeps boxplot in a color and all the data outside the box in black. Not as nice/clean
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #doge means that dots do not overlay but move to the side making it wider where there are more points
print(Nutrientplot4)

# Violin plot - shows density along with spread
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
print(Nutrientplot6) #missing a legend so not a good way of visualizing easier way is using the next one. 
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) #fill made color under the curves
print(Nutrientplot6)
