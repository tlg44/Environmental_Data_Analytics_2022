...
matrix1[4, ] #pulls everything in 4th row when left blank
matrix1[ , 3] #everything from 3rd column
matrix1[c(12, 14)] #pulls just 12 and 14
matrix1[c(12:14)] #pulls 12 through 14
matrix1[2:4, 1:3] #
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # this is used to rename the columns from their original vector names
...
```{r}
HighTempF <- c(41, 43, 46, 36, 42, 49, 52, 56, 67, 60)
LowTempF <- c(20, 25, 28, 19, 23, 25, 29, 43, 49, 36)
...
```{r}
HighTempC <- c(5, 6, 7, 2, 6, 9, 11, 13, 19, 15)
LowTempC <- c(-7, -4, -3, -8, -5, -4, -2, 6, 10, 2)

f_to_c = function(f) {(f-32)*(5/9) }

highs_C <- f_to_c(HighTempF)
lows_C <- f_to_c(LowTempF)
...
```{r}
TenDayForcast <- data.frame(HighTempF, LowTempF, HighTempC, LowTempC)

DFColNames <- c("HighF", "LowF", "HighC", "LowC")
...
summary(TenDayForcast)
sd(TenDayForcast$HighTempF)
