...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #like the text to columns code in excel, the space is the delimiter. null2 is just the word average. saw a space in the statistics column before month so needed 3 columns to decatenate. 
  mutate( Date = my(paste0(Month,"-",Year))) %>% #like concatenate in excel
...
BRA_complete <- inner_join(Rainfall_BR_processed,Temp_BR_processed) #joined by date
...
plot.rain <- ggplot(BRA_complete, aes(x = Date, y = Rainfall)) + 
  geom_line() + 
  geom_smooth(method = "lm")
print(plot.rain)

plot.temp <- ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) + 
  geom_line() + 
  geom_smooth(method = "lm")
print(plot.temp)
...
BRA_Rain_TS <- ts(Rainfall_BR_processed$Rainfall, start = c(f_year, f_month), frequency = 12)

#tineseries for temp
f_month_temp <- month(first(Temp_BR_processed$Date))
f_year_temp <- year(first(Temp_BR_processed$Date))
BRA_Temp_TS <- ts(Temp_BR_processed$Temperature_C, start = c(f_year_temp, f_month_temp), frequency = 12)


#code from professor the order of fyear and fmonth matters! 
fmonth <- month(first(BRA_complete$Date))
fyear <- year(first(BRA_complete$Date))

BR_Rain_ts <- ts(BRA_complete$Rainfall, start = c(fyear, fmonth), frequency = 12)
BR_Temp_ts <- ts(BRA_complete$Temperature_C, start = c(fyear, fmonth), frequency = 12)
...
BRA_Rain_TS_Decomp <- stl(BRA_Rain_TS, s.window = "periodic")
plot(BRA_Rain_TS_Decomp)

BRA_Temp_TS_Decomp <- stl(BRA_Temp_TS, s.window = "periodic")
plot(BRA_Temp_TS_Decomp)

BR_Rain_Components <- mutate(BRA_Rain_TS_Decomp$time.series [,1:3])

BR_Temp_Components <- mutate(BRA_Temp_TS_Decomp$time.series [,1:3])

BR_Rain_Components <- mutate(BRA_Rain_Components,
