...
EPAair.subsample <- sample_n(EPAair, 5000) #shaprio tests doesn't like more than 5K data points
...
#geom_density
ozone.by.yr <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) + 
  geom_density(alpha = 0.5) + labs(title= "Density Plot: Ozone by Year", 
                                   y = "Density", x= "Ozone AQI Value", 
                                   fill = "Year") + 
...
#bocplot
ozone.by.yr.box <- ggplot(EPAair, aes(x= as.factor(Year), y = Ozone)) + 
  geom_boxplot() + stat_summary(geom = 'text', fun = max, vjust = -1, 
                                size = 4, label = c("a","b"), color='red') + 
  ylim(0,150) + 
  labs(x = "", y = 'Ozone AQI Value')
                                                                                                      
print(ozone.by.yr.box)


#Violin
ozone.violin <- ggplot(EPAair, aes(x= as.factor(Year), y = Ozone)) + 
  geom_violin(draw_quantiles = 0.5) + stat_summary(geom = 'text', fun.y = max, vjust = -1, size = 4, label = c("a","b"), color='red') + labs(x = "", y = 'Ozone AQI Value')
...
ozone.lm <- ggplot(EPAair, x = Ozone, y = PM2.5) + geom_point() + geom_smooth(method = 'lm')
print(ozone.lm)

shapiro.test(EPAair.subsample$PM2.5)#failed
hist(EPAair$PM2.5) #possible tail
qqnorm(EPAair$PM2.5); qqline(EPAair$PM2.5) #a lot of zeros

lm.test <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(lm.test)


#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 
ggplot(EPAair, aes(x=Ozone,y=PM2.5)) + geom_point() + geom_smooth(method = 'lm')
...
#plot the matrix
corrplot(the_cor, upper = 'ellipse')
...
the_lm6 <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + meanLat)
summary(the_lm6)
...
the_lm7 <- lm(data = EPAair, PM2.5 ~ Ozone + Month + meanLat)
summary(the_lm7)
